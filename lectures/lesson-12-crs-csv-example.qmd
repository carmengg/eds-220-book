...
```python
updated_df = geo_df.to_crs(new_crs)
```
where:
- `updated_df` is the output of the method,
- `geo_df` is the `gpd.GeoDataFrame` we want to transform, 
...
Nowe we can visually confirm there are no power statoins in the western islands of Alaska:
