...
In the previous histogram we can see there might be two different populations. This is not surprising since we know these are flipper lengths for penguins of three different species, two sexes and three islands, all of which could affect the flipper lenghts. So let's say we want to plot histograms of the flipper lengths by species on the same axis. How would we do this `by hand`?
...
2. We create a histogram for the flipper length of each species.

3. Then we combine the histograms for each species into a single plot.
...
2. **Apply**: Calculate some summary statistic on each group (e.g. histogram per species, number of individuals per island, flipper length by sex and species)

3. **Combine**: Combine the statistic calculated on each group back together.

In Python we can use the `groupby()` method to do split (or group) the data into different categories. The general syntax for `groupby()` is
...
Create a scatter plot of the flipper length agains body mass (x=flipper length, y=body mass) by sex. 
