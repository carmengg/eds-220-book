...
- We then convert this `pd.Series` into a `pd.DataFrame` and clean it up a bit.
 ...
 We can  select this data using the `isin()` method for `pd.Series` inside the locator brackets:
...
To finish, we can use `matplotlib` to customize our graph:
