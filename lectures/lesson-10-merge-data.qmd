...
This dataset comes as a shapefile we need to download. 
[Natural Earth](https://www.naturalearthdata.com) is a public domain dataset with ready-to-use data for creating maps. 

The second dataset we will use is [list of Arctic communities and their location (Brook, 2023)](https://search.dataone.org/view/doi%3A10.18739%2FA28S4JQ80) which can be accessed through the DataONE repository. 
...
# select Scandi countries and adming, type, and geometry columns
...
We can verify that the 'Aland Islands' value does not occur anywhere in the dataframe:
...
This makes it easy to take a quick look at our data by plotting it on top of the countries dataframe:
...
- We then convert this `pd.Series` into a `pd.DataFrame` and clean it up a bit.
 ...
 Since we only want data from Scandinavia, we can use the codes for this countries to locate these rows:
...
Conveniently, the `scandi_names` list is in the same order as the country codes in the `scandi_n_comms` dataframe, so we can readily add a new column:
