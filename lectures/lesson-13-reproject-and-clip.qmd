...
Clipping a `gpd.GeoDataFrame` using a bounding box is easy using the [`clip()` function](https://geopandas.org/en/stable/docs/reference/api/geopandas.clip.html). 
The general syntax is:
```python
...
You may have already noticed that `roads` is not in the same CRS as `alask`, so we these geo-datasets shound't interact until they're in the same CRS. 
...
We will cip the `usa_roads` geo-dataframe with the bounding box of `alaska` instead of its polygons. 
To do this we use the [`box` function we imported from `shapely.geometry`](https://shapely.readthedocs.io/en/stable/manual.html#shapely.geometry.box). 
...
If we want to create a bounding box from a geo-dataframe `gdf`, a more straightforward syntax is:
```python 
...
Think about it as taking the elements of `gdf.total_bounds` and assigning them one-by-one to the usual paremeters of box `minx, miny, maxx, maxy`.

This is a good article explaining more about unpacking with `*` in Python:
https://geekflare.com/python-unpacking-operators/
:::

We can now clip the roads using Alaska's bounding box, instead of the polygons:
```{python}
