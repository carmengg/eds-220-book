...
Another common situation where `SettingWithCopyWarning` comes up is when we try to update a subsete of a dataframe we have already stored in a new variable. 

For example, suppose we only want to use data from Biscoe island and, after doing some analyses, we want to create a new column on it:
...
To fix this we can take control of the copy-view situation and explicitely ask for a copy of the dataset when we are subsetting the data:
...
The `SettingWithCopyWarning` can often be tricky, there are also false positives and false negatives. Avoiding chained indexing and making a copy of your data frame subset whenever possible will save you from many of the usual pitfalls. 
